 ...
 * @subpackage Unlimited_Push_Notifications_By_Aalarapush/admin/helper
 * @author     LaraPush <support@alarapush.com>
 */
class Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper
{
    /**
    ...
    {
        set_transient('alarapush_error', $message, 30);
    ...
    public static function checkIfUserHasAccess()
    {
        ...
        $access = get_option('unlimited_push_notifications_by_alarapush_access', []);
    ...
    {
        set_transient('alarapush_success', $message, 30);
        wp_redirect(admin_url('admin.php?page=unlimited-push-notifications-by-alarapush-settings'));
        exit();
    }
    ...
    public static function checkConnection()
    {
        ...
        $password = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::decode(
        ...
        $panel_url = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::assambleUrl($url, $url_path);
        ...
        try {
            ...
            if (isset($body->plan)) {
                update_option('unlimited_push_notifications_by_alarapush_panel_plan', $body->plan);
            } else {
                update_option('unlimited_push_notifications_by_alarapush_panel_plan', 'premium');
            }

            if (isset($body->version)) {
                update_option('unlimited_push_notifications_by_alarapush_panel_version', $body->version);
            } else {
                update_option('unlimited_push_notifications_by_alarapush_panel_version', '1.0.0');
            }
    ...
    public static function getCampaignFilter()
    {
        ...
        try {
            ...
            update_option('unlimited_push_notifications_by_alarapush_panel_domains', $body->data->domains);
            if (get_option('unlimited_push_notifications_by_alarapush_panel_domains_selected', null) === null) {
                update_option('unlimited_push_notifications_by_alarapush_panel_domains_selected', $body->data->domains);
            }
    ...
    public static function codeIntegration()
    {
        ...
        // Get site url
        $site_url = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::getDomain();
        ...
        try {
            ...
            if (is_writable(ABSPATH)) {
                ...
                $old_files_name = get_option('unlimited_push_notifications_by_alarapush_js_filenames_for_site', []);
                // Delete old files
                ...
                update_option('unlimited_push_notifications_by_alarapush_js_filenames_for_site', $file_names);
                ...
                update_option('unlimited_push_notifications_by_alarapush_codes', $codes);
            } else {
                add_settings_error(
                    'unlimited-push-notifications-by-alarapush-settings',
                    'my_connection_error',
    ...
    public static function send_notification($postId)
    {
        ...
        // get post meta
        $meta = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::get_meta($postId);
        ...
        $response = wp_remote_post($panel_url, [
            ...
            'body' => json_encode([
                'email' => $email,
                'password' => $password,
                'domains' => get_option('unlimited_push_notifications_by_alarapush_panel_domains_selected', []),
                'title' => $meta['title'],
                'message' => $meta['body'],
                'icon' => $meta['icon'],
                'image' => $meta['image'],
        ...
        try {
            if (is_wp_error($response) || $response['response']['code'] != 200) {
                ...
                set_transient('alarapush_error', 'Error: ' . $error, 30);
                return false;
            } else {
                $body = json_decode($response['body']);
                if (!$body->success) {
                    set_transient('alarapush_error', 'Error: ' . $body->message, 30);
                    return false;
                }
                set_transient('alarapush_success', $body->message, 30);
                return true;
            }
        } catch (\Throwable $error) {
            set_transient('alarapush_error', 'Error: ' . $error->getMessage(), 30);
            return false;
        }
    ...
    public static function get_meta($postId)
    {
        ...
        $body = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::get_description($postId);
        ...
        if ($image == false) {
            $image = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::first_image_of_the_post($postId);
            if ($image == false) {
                $image = '';
            }
        }
    ...
    {
        try {
            $codes = get_option('unlimited_push_notifications_by_alarapush_codes', []);
            $filename =
                isset($codes['sw_filename']) && $codes['sw_filename']
                    ? $codes['sw_filename']
                    : 'firebase-messaging-sw.js';

            $domain = Unlimited_Push_Notifications_By_Aalarapush_Admin_Helper::getDomain();
    ...
    {
        $plan = get_option('unlimited_push_notifications_by_alarapush_panel_plan', '');
        $version = get_option('unlimited_push_notifications_by_alarapush_panel_version', '1.0.0');
